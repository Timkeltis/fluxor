...
- **前端**：Vue 3 + TypeScript 构建，提供高性能的客户端交互体验，源码位于 `frontend/` 目录，由后端在 `backend/` 内嵌其构建产物。
...
### 2. 整体构建
推荐使用项目根目录的 Makefile 完成前端构建 + 同步 + 后端编译：
```bash
# 完整构建（构建前端 → 同步 dist → 编译后端，输出 ./fluxor）
make

# 仅构建前端 (frontend/dist)
make frontend

# 仅同步 dist 到 backend/dist
make sync

# 仅编译后端二进制（依赖 dist 已同步）
make backend
