...
graph TD
    ...
    B -.->|SSE: 内核状态| F
    F -->|Unix Socket + Bearer Token| C[mi鸿蒙 内核]
...
| 通道 | 协议 | 用途 | 说明 |
|------|------|------|------|
| 管理接口 | HTTP | 配置读写、订阅管理、内核启停 | 全部经后端中转，自动附加认证头 |
...
* 否则「保存一次订阅配置」会把 TProxy 的例外列表静默清空。
...
* **认证头自动附加**：前端请求内核 API 时，后端自动补 `Bearer Token`，密钥不下发给浏览器。
